...
class AdvisorPlan extends Component
{
    public $advisee;
    public $year;
    public $subjects;
    ...
    {
        $advisee = advisee::find($this->advisee)->first(); 
        if ($advisee) {
    ...
    private function getPreviousSemester($semester)
    {
        ...
        $advisee = advisee::find($this->advisee)->first();
        $intake = advisee::where('advisee_id', $advisee->advisee_id)->value('cs_id');
    ...
    public function updateGrade($subjectCode, $grade, $semesterName)
    {
        ...
        // Calculate the GPA for the specific semester
